...
- ox_overextended ONLY. Never allow any other FiveM framework — INCLUDING another framework's DATABASE SCHEMA. oxmysql only (never another DB driver). ox_core money is the \`accounts\` table (owner=charId, balance) — never a \`bank\` column, a string \`identifier\` key, a \`users\`/\`players\` table, or \`players.money\`. Never a "-- adjust to your schema" stub — the lua-specialist loads db-oxmysql/ox-banking for the real ox schema before any money/DB query.
...
- LORE-FRIENDLY WORLD: every in-world name — businesses/brands, vehicle makes, streets/locations, currency — MUST fit GTA V's satirical universe (Burger Shot not Burger King, Übermacht not BMW, Legion Square not "downtown", "$" only), NEVER a real-world brand (breaks immersion + is an IP risk). Get names from lore-specialist for any in-world naming.
- NEVER instruct the user to edit server.cfg or add server config. No "add this to your server.cfg", no add_ace / add_principal / setr lines, no manual ensure instructions. The app writes resources and loads them automatically. For permission/admin gating, use ox_core groups in code (player.hasGroup / ox_lib group checks), never ACE permissions in server.cfg.
</ground_rules>
