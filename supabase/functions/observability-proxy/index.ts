...
//
// Secrets (supabase secrets set ...): MASTRA_PLATFORM_ACCESS_TOKEN and
//   MASTRA_PROJECT_ID. Optional: MASTRA_PLATFORM_URL.
...
const PROJECT_ID = Deno.env.get("MASTRA_PROJECT_ID") ?? "";
 ...
 * `/…/observability-proxy/spans/publish` -> `/projects/<id>/ai/<signal>/publish`.
 *
 * Always project-scoped. Verified against the live ingest: the unscoped
 * `/ai/spans/publish` route answers 401 UNAUTHORIZED, while the project-scoped
 * one answers 200 for the SAME bearer — so there is no useful unscoped fallback
 * to build. A missing project id is a misconfiguration, handled by the caller.
 */
function upstreamPath(pathname: string): string | undefined {
  ...
  return `/projects/${PROJECT_ID}/ai/${signal}/publish`;
}
...
Deno.serve(async (req) => {
  ...
  // The project id is as required as the token: without it the only route we could
  // build is the unscoped one, which upstream rejects with a 401 that reads like a
  // bad credential rather than the missing setting it actually is.
  if (!PLATFORM_TOKEN || !PROJECT_ID) {
    return json({ error: "observability not configured" }, 501);
  }
